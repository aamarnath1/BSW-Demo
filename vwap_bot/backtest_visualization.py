...
class VWAPBacktestVisualizer:
    ...
    def plot_results(self, results, symbol):
        ...
        ax1.set_title(f'{symbol} Price and VWAP')
        ax1.legend()
        ax1.grid(True)
...
if __name__ == "__main__":
    ...
    if all_stats:
        ...
        print("\nStrategy Performance Comparison (Last Year):")
        print(tabulate(all_stats, headers=headers, tablefmt='grid'))
        
        # Save summary statistics
        summary_df = pd.DataFrame(all_stats, columns=headers)
        summary_df.to_csv('backtest_summary_yearly.csv', index=False)
        summary_df.to_excel('backtest_summary_yearly.xlsx', index=False)
        ...
        if all_trades:
            ...
            trades_df.to_csv('backtest_trades_yearly.csv', index=False)
            trades_df.to_excel('backtest_trades_yearly.xlsx', index=False)
            ...
            analysis_df.to_csv('backtest_analysis_yearly.csv', index=False)
            analysis_df.to_excel('backtest_analysis_yearly.xlsx', index=False)
            
            print("\nDetailed analysis saved to:")
            print("- backtest_summary_yearly.csv/xlsx (Overall performance by symbol)")
            print("- backtest_trades_yearly.csv/xlsx (Individual trade data)")
            print("- backtest_analysis_yearly.csv/xlsx (Detailed strategy analysis)") 